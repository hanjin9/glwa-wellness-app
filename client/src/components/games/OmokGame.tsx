...
export default function OmokGame({ onGameEnd }: OmokGameProps) {
  ...
  const handleCellClick = (row: number, col: number) => {
    ...
    if (checkWin(newBoard, row, col, 1)) {
      setBoard(newBoard);
      setGameResult('win');
      const points = difficulty === 'easy' ? 200 : difficulty === 'medium' ? 350 : 500;
    ...
    // AI 움직임 (간단한 랜덤)
    let aiMoved = false;
    for (let i = 0; i < 15 && !aiMoved; i++) {
      for (let j = 0; j < 15 && !aiMoved; j++) {
        if (newBoard[i][j] === 0 && Math.random() < 0.3) {
    ...
    return (
      <Card className="p-6 max-w-md mx-auto">
        <div className="space-y-4">
          <h2 className="text-2xl font-bold text-center">⭕ 오목</h2>
          ...
          <div className="bg-blue-50 p-3 rounded-lg text-sm text-blue-700">
            💡 <strong>팁:</strong> 5개를 먼저 연결하면 승리합니다!
          </div>
        </div>
      </Card>
    );
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold">⭕ 오목</h2>
            </div>
                ...
                {board.map((row, rowIdx) =>
                  row.map((cell, colIdx) => (
                    <button
                      ...
                      className={`
                        w-8 h-8 border border-amber-800
                        flex items-center justify-center
                        ${phase === 'completed' || cell !== 0 ? 'cursor-not-allowed' : 'cursor-pointer hover:bg-amber-200'}
                      `}
                    >
                      {cell === 1 && <div className="w-6 h-6 bg-black rounded-full" />}
                      {cell === 2 && <div className="w-6 h-6 bg-white rounded-full border-2 border-gray-400" />}
                    </button>
                  ))
                )}
              </div>
          ...
          {/* 팁 */}
          <div className="bg-blue-50 p-3 rounded-lg text-sm text-blue-700">
            💡 <strong>팁:</strong> 5개를 먼저 연결하세요! (검은 돌: 당신, 흰 돌: AI)
          </div>
        </div>
      </Card>
    );
